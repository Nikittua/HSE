...
int main() {
    ...
    ret = fread(buf, 1, sizeof(buf), pipe);// возвращает количество действительно считанных объектов 1= 1 байт

    ret = sendto(sock_id, buf, ret, 0, (struct sockaddr*)&server, server_size); // отправляем серверу количество байтов ret в buf.
    if (ret < 0) {
